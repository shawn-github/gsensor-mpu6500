...
static int sensor_initial(struct i2c_client *client)
{
	...
	int result = 0;
		...
		printk("%s:start to init sensor %d\n",__func__,__LINE__);
	...
	if(result < 0)
	{
		...
		printk("%s:success to init sensor %d\n",__func__,__LINE__);
...
static int sensor_chip_init(struct i2c_client *client)
{
	...
	printk("%s:%s:devid=0x%x,ops=0x%p %d\n",__func__, sensor->i2c_id->name, sensor->devid,sensor->ops,__LINE__);
...
static int sensor_irq_init(struct i2c_client *client)
{
	...
	{		
		...
		printk("mpu6500:%s:use polling,delay=%d ms\n",__func__,sensor->pdata->poll_delay_ms);
	}

    printk("mpu6500:%s success %d\n",__func__,__LINE__);
error:	
	return result;
}
...
static int sensor_misc_device_register(struct sensor_private_data *sensor, int type)
{
	...
	printk("%s:miscdevice: %s %d\n",__func__,sensor->miscdev.name,__LINE__);
			...
			struct sensor_operate *(*get_sensor_ops)(void))
{
	...
	printk("%s:%s,id=%d %d\n",__func__,sensor_ops[ops->id_i2c]->name, ops->id_i2c,__LINE__);
	return result;
}
